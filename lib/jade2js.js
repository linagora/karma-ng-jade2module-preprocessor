...
var createJade2JsPreprocessor = function(logger, basePath, config) {
  ...
  return function(content, file, done) {
    ...
    var jadeListOptionsAndLocals = {
      options: ['basedir', 'doctype', 'pretty', 'filters', 'self', 'debug', 'compileDebug',
                'globals', 'cache', 'inlineRuntimeFunctions', 'name'],
      locals: ['__']
    };
    var jadeCompile = {
      options: _.extend({ filename: file.originalPath }, _.pick(config.jadeRenderConfig, jadeListOptionsAndLocals.options)),
      locals: _.pick(config.jadeRenderConfig, jadeListOptionsAndLocals.locals)
    };

    content = jade.compile(content, jadeCompile.options)(jadeCompile.locals);
